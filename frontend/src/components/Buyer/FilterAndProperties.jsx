...
const FilterAndProperties = () => {
 ...
 const updateFiltersFromVoiceInput = async (transcript) => {
   try {
           ...
           contents: [
             {
               parts: [
                 {
                   text: `Extract the main property filters from the following sentence: ${transcript}. The property types are Flat, House, Plot, PG. The cities are Nagpur, Pune, Mumbai, Amravati. Extract the property type, city, price range, and any other relevant filters. Return the response in JSON format.`,
                 },
               ],
             },
           ],
     ...
     console.log("Extracted filter data",extractedFilters)
...
const parseFiltersFromGeminiResponse = (data) => {
  ...
  // Handle "under X lakh/thousand" or "below X lakh/thousand"
  const underPriceRegex = /(under|below|less than)\s*(\d+)\s*(lakh|thousand)?/i;
  const underMatch = responseText.match(underPriceRegex);

  if (underMatch) {
    const maxPrice = parseInt(underMatch[2], 10);
    const maxUnit = underMatch[3] ? underMatch[3].toLowerCase() : "lakh"; // Default unit = lakh

    filters.maxPrice = maxUnit === "lakh" ? maxPrice * 100000 : maxPrice * 1000;
  } else {
    // Extract price range correctly
    const priceRangeRegex =
      /(\d+)\s*(lakh|thousand)?(?:\s*to\s*(\d+)\s*(lakh|thousand)?)?/i;
    const priceMatch = responseText.match(priceRangeRegex);

    if (priceMatch) {
      let minPrice = parseInt(priceMatch[1], 10);
      let minUnit = priceMatch[2] ? priceMatch[2].toLowerCase() : "lakh"; // Default unit = lakh
      let maxPrice = priceMatch[3] ? parseInt(priceMatch[3], 10) : null;
      let maxUnit = priceMatch[4] ? priceMatch[4].toLowerCase() : minUnit; // Default maxUnit same as minUnit

      // Convert prices correctly
      filters.minPrice =
        minUnit === "lakh" ? minPrice * 1 : minPrice * 1;

      if (maxPrice) {
        filters.maxPrice =
          maxUnit === "lakh" ? maxPrice * 100000 : maxPrice * 1000;
      } else {
        filters.maxPrice =
          filters.minPrice + (minUnit === "lakh" ? 1000000 : 10000);
      }
    }
  }
